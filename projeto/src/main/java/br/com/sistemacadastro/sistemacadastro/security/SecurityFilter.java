...
public class SecurityFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        ...
        if(login != null){
            Collaborator collaborator = repository.findByEmail(login).orElseThrow(()-> new RuntimeException("User not found"));
            var authorities = Collections.singletonList(new SimpleGrantedAuthority("ADMIN"));
            var authentication = new UsernamePasswordAuthenticationToken(collaborator, null, authorities);
